...
coinpunk.router.map('#/backup').to(function () {
    coinpunk.router.initWallet(function (res) {
        if (res == false)
            return;
        coinpunk.router.render('workspace', 'backup');
    });
});
...
coinpunk.router.map('#/backup/download').to(function () {
    coinpunk.router.initWallet(function (res) {
        ...
        saveAs(blob, "coincosm-wallet.txt");
...
coinpunk.router.map('#/accounts/import').to(function () {
    if (coinpunk.wallet) {
        ...
        if (window.File && window.FileReader && window.FileList && window.Blob) {
            coinpunk.router.render('workspace', 'accounts/import');
        } else {
...
coinpunk.router.map('#/account/settings').to(function () {
    coinpunk.router.initWallet(function (res) {
        if (res == false)
            return;
        coinpunk.router.render('workspace', 'accounts/settings');
    });
});
...
coinpunk.router.map('#/buy').to(function () {
    coinpunk.router.initWallet(function (res) {
        if (res == false)
            return;
        coinpunk.router.render('workspace', 'buy');
    });
});
